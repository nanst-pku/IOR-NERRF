...
def test_loss(loss, tonemapper):
	img_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	img_ref = img_cuda.clone().detach().requires_grad_(True)
	target_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
