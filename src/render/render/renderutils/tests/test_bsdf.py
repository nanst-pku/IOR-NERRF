...
def test_normal():
	pos_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	pos_ref = pos_cuda.clone().detach().requires_grad_(True)
	view_pos_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	view_pos_ref = view_pos_cuda.clone().detach().requires_grad_(True)
	perturbed_nrm_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	perturbed_nrm_ref = perturbed_nrm_cuda.clone().detach().requires_grad_(True)
	smooth_nrm_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	smooth_nrm_ref = smooth_nrm_cuda.clone().detach().requires_grad_(True)
	smooth_tng_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	smooth_tng_ref = smooth_tng_cuda.clone().detach().requires_grad_(True)
	geom_nrm_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	geom_nrm_ref = geom_nrm_cuda.clone().detach().requires_grad_(True)
	target = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0))
...
def test_schlick():
	f0_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	f0_ref = f0_cuda.clone().detach().requires_grad_(True)
	f90_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	f90_ref = f90_cuda.clone().detach().requires_grad_(True)
	cosT_cuda = torch.rand(1, RES, RES, 1, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True) * 2.0
...
def test_ndf_ggx():
	alphaSqr_cuda = torch.rand(1, RES, RES, 1, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	alphaSqr_cuda = alphaSqr_cuda.clone().detach().requires_grad_(True)
	alphaSqr_ref = alphaSqr_cuda.clone().detach().requires_grad_(True)
	cosT_cuda = torch.rand(1, RES, RES, 1, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True) * 3.0 - 1
	cosT_cuda = cosT_cuda.clone().detach().requires_grad_(True)
	cosT_ref = cosT_cuda.clone().detach().requires_grad_(True)
	target = torch.rand(1, RES, RES, 1, dtype=DTYPE, device=torch.device("cuda:%d" % 0))
...
def test_masking_smith():
	alphaSqr_cuda = torch.rand(1, RES, RES, 1, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	alphaSqr_ref = alphaSqr_cuda.clone().detach().requires_grad_(True)
	cosThetaI_cuda = torch.rand(1, RES, RES, 1, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	cosThetaI_ref = cosThetaI_cuda.clone().detach().requires_grad_(True)
	cosThetaO_cuda = torch.rand(1, RES, RES, 1, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
...
def test_lambert():
	normals_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	normals_ref = normals_cuda.clone().detach().requires_grad_(True)
	wi_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
...
def test_frostbite():
	...
	wi_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	wi_ref = wi_cuda.clone().detach().requires_grad_(True)
	wo_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	wo_ref = wo_cuda.clone().detach().requires_grad_(True)
	rough_cuda = torch.rand(1, RES, RES, 1, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
...
def test_pbr_specular():
	col_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	col_ref = col_cuda.clone().detach().requires_grad_(True)
	nrm_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	nrm_ref = nrm_cuda.clone().detach().requires_grad_(True)
	wi_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	wi_ref = wi_cuda.clone().detach().requires_grad_(True)
	wo_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	wo_ref = wo_cuda.clone().detach().requires_grad_(True)
	alpha_cuda = torch.rand(1, RES, RES, 1, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
...
def test_pbr_bsdf(bsdf):
	kd_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	kd_ref = kd_cuda.clone().detach().requires_grad_(True)
	arm_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	arm_ref = arm_cuda.clone().detach().requires_grad_(True)
	pos_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	pos_ref = pos_cuda.clone().detach().requires_grad_(True)
	nrm_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	nrm_ref = nrm_cuda.clone().detach().requires_grad_(True)
	view_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	view_ref = view_cuda.clone().detach().requires_grad_(True)
	light_cuda = torch.rand(1, RES, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
