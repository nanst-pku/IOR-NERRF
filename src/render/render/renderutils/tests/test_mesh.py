...
def test_xfm_points():
	points_cuda = torch.rand(1, RES, 3, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
	points_ref = points_cuda.clone().detach().requires_grad_(True)
	mtx_cuda = torch.rand(BATCH, 4, 4, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=False)
	mtx_ref = mtx_cuda.clone().detach().requires_grad_(True)
	target = torch.rand(BATCH, RES, 4, dtype=DTYPE, device=torch.device("cuda:%d" % 0), requires_grad=True)
