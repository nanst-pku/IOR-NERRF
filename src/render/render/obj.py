...
def load_obj(filename, clear_ks=True, mtl_override=None):
    ...
    all_materials = [
        {
            'name' : '_default_mat',
            'bsdf' : 'pbr',
            'kd'   : texture.Texture2D(torch.tensor([0.5, 0.5, 0.5], dtype=torch.float32, device=torch.device("cuda:%d" % 0))),
            'ks'   : texture.Texture2D(torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32, device=torch.device("cuda:%d" % 0)))
        }
    ]
    ...
    vertices = torch.tensor(vertices, dtype=torch.float32, device=torch.device("cuda:%d" % 0))
    texcoords = torch.tensor(texcoords, dtype=torch.float32, device=torch.device("cuda:%d" % 0)) if len(texcoords) > 0 else None
    normals = torch.tensor(normals, dtype=torch.float32, device=torch.device("cuda:%d" % 0)) if len(normals) > 0 else None
    
    faces = torch.tensor(faces, dtype=torch.int64, device=torch.device("cuda:%d" % 0))
    tfaces = torch.tensor(tfaces, dtype=torch.int64, device=torch.device("cuda:%d" % 0)) if texcoords is not None else None
    nfaces = torch.tensor(nfaces, dtype=torch.int64, device=torch.device("cuda:%d" % 0)) if normals is not None else None
