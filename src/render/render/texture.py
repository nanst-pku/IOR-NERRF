...
class Texture2D(torch.nn.Module):
    ...
    def __init__(self, init, min_max=None):
        ...
        if isinstance(init, np.ndarray):
            init = torch.tensor(init, dtype=torch.float32, device=torch.device("cuda:%d" % 0))
...
def _load_mip2D(fn, lambda_fn=None, channels=None):
    imgdata = torch.tensor(util.load_image(fn), dtype=torch.float32, device=torch.device("cuda:%d" % 0))
