...
class NeRFRenderer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.use_progressive_encoder = True
        self.enable_refr = enable_refr
        self.enable_refl = enable_refl
        self.n_coarse = n_coarse
        self.ior = ior
        self.ior_range = [ior-0.3,ior+0.3]
        self.n_fine = n_fine
        ...
        if self.use_grid:
            ...
            if stage == 1:
                ...
                mesh = trimesh.load("data/init/sphere.obj", force="mesh")
                scale = (
                    1.5 / np.array(mesh.bounds[1] - mesh.bounds[0]).max()
                )  # if use eikonal dataset, change 1.0 to 1.5
                center = np.array(mesh.bounds[1] + mesh.bounds[0]) / 2
    ...
    def init_tet(self, mesh_path, geometry_name=None):
        ...
        try:
            ...
            # init sdf
            BVH = cubvh.cuBVH(mesh.vertices, mesh.faces)#层次包围盒
            ...
            self.sdf.data += sdf.to(self.sdf.data.dtype).clamp(-1, 1)#将sdf的值限制在-1到1之间
            sdf = self.sdf
        ...
        face_normals = torch.cross(v1 - v0, v2 - v0) #叉乘计算法向量
    ...
    def intersection_with_mesh(self, rays, mesh, far=False):
        ...
        if self.need_trace==False:#不需要光线追踪，直接进行采样，全部保持原始值
            return torch.zeros_like(rays[:, 0], dtype=torch.bool), torch.zeros(rays.shape[0]), torch.zeros(rays.shape[0],3), torch.tensor([])
        import time
        ...
        need_trace=True #不提供mask，需要完整追踪
        if rays.shape[1] == 9:
            #print('mask')
            valid_mask = rays[:,-1]
            ...
            #print('no mask')
            valid_mask = torch.ones(ray_num, dtype=torch.bool)
        ...
        for i in range(ray_num):
            if not need_trace and not valid_mask[i]:
                continue
    ...
    def trace_ray_mesh(
        ...
        if len(intersection_face) == 0:
            ...
            normals = face_normals[intersection_face]#相交面的法向量
        normals = normals.to(device=rays.device)
        valid_mask = valid_mask.to(device=rays.device)#True/false
        hit_rays = rays[valid_mask]# [num_hit_ray,8]
        #删除mask信息
        hit_rays = hit_rays[:, :8]
        rays=rays[:, :8]

        intersection_depth = intersection_depth.to(device=rays.device)
        init_depth = intersection_depth[valid_mask].to(device=rays.device)
        hit_pts = hit_rays[..., :3] + init_depth.unsqueeze(1) * hit_rays[..., 3:6]#交点坐标
        in_dir = hit_rays[..., 3:6]
        ...
        refr_rays = torch.zeros_like(hit_rays)#折射光线[num_hit_ray,8]
        
        refr_rays[:, :3] = hit_pts#折射光线起点 [num_hit_ray,3]
        n_ = 1 / n
        ...
        refr_rays[:, 3:6] = refr_dir#折射光线方向[num_hit_ray,3]
        fresnel_1 = self.Fresnel_term(n_, in_dir, refr_dir, normals)#菲涅尔反射系数,[num_hit_ray,1]
        ...
        (
            ...
            normals_,
            intersection_face_,
        ) = self.intersection_with_mesh(refr_rays, mesh, far=True)#第二次相交
        normals_ = normals_.to(device=rays.device)#第二次相交的法向量,[num_out_ray,3],通常情况下，num_out_ray=num_hit_ray
        ...
        # second refraction
        refr_rays_ = torch.zeros_like(hit_rays)#第二次折射光线起点
        ...
        refr_rays_[:, 3:6] = refr_dir_#第二次折射光线方向
        fresnel_2 = self.Fresnel_term(n, in_dir_, refr_dir_, normals_)#菲涅尔反射系数,[num_hit_ray,1]
        fresnel_2=1-fresnel_2
        fresnel_2[fresnel_2<0.7]=0.7
        fresnel_2_total=torch.ones_like(fresnel_1)

        #处理不闭合的表面
        # dealing with rays missing the second intersection (unwatertight surface) #处理第二次相交丢失的光线
        ...
        fresnel_2_total=copy_index(fresnel_2_total,valid_mask_.to(device=rays.device),fresnel_2[valid_mask_])
        fresnel_2=fresnel_2_total
        # update depth: intersection with planes
        refr_rays_[:, 7] = rays[valid_mask][:, 7] - init_depth - zdepth#更新深度
    ...
    def composite(self, model, rays, z_samp, coarse=True, sb=0):
        ...
        with profiler.record_function("renderer_composite"):
            B, K = z_samp.shape
            deltas = z_samp[:, 1:] - z_samp[:, :-1]  # 间隔？
            delta_inf = rays[:, -1:] - z_samp[:, -1:] # 最后一个点到far的距离
            ...
            val_all = []
            #采样光线上每个点均为一组数据，取全部的点作为输入，因此会出现新的batch_size
            if sb > 0:
    ...
    def forward(
        ...
        with profiler.record_function("renderer_forward"):
            ...
            assert len(rays.shape) == 3 #1,batch,8
            superbatch_size = rays.shape[0]
            last_dim = rays.shape[-1]
            rays = rays.reshape(-1, last_dim)  # (SB * B, 8)
            ...
            if reflection or refraction:
                has_reflection=reflect_rays is not None and not reflect_rays.shape[0]==0
                if not reflection_only and has_reflection:
                    #print('refr+refl')
                    if reflection:
                    ...
                    if refraction and fresnel is not None and has_reflection:
                        rgb = coarse_composite[1]
                        rgb[valid_mask] = fresnel.unsqueeze(-1)* rgb[valid_mask] * fresnel_.unsqueeze(-1)
                        if reflection:
                            rgb[valid_mask] = (
                                rgb[valid_mask]
