...
def contract_to_unisphere(
    ...
    mag = torch.linalg.norm(x, ord=ord, dim=-1, keepdim=True)#定义转换到球体的范数
    mask = mag.squeeze(-1) > 1 #删掉最后一维，变为一维后，取出大于1的值

    if derivative:
        dev = (2 * mag - 1) / mag**2 + 2 * x**2 * (
            1 / mag**3 - (2 * mag - 1) / mag**4
        )#导数
        dev[~mask] = 1.0#小于1的导数为1
        dev = torch.clamp(dev, min=eps)#导数的最小值为eps
        return dev
    else:
        x[mask] = (2 - 1 / mag[mask]) * (x[mask] / mag[mask])#无穷范数
        x = x / 4 + 0.5  # [-inf, inf] is at [0, 1]，将[-1,1]映射到[0,1]
        return x
...
class NGPRadianceField(torch.nn.Module):
    ...
    def _query_rgb(self, dir, embedding, apply_act: bool = True):
        ...
        rgb = self.mlp_head(h)
        rgb=rgb.reshape(list(embedding.shape[:-1]) + [3])
        rgb=rgb.to(embedding)
        if apply_act:
