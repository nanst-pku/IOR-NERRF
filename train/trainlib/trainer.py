...
class Trainer:
    def __init__(self, net, train_dataset, test_dataset, args, conf, device=None):
        ...
        self.num_epochs = 8 #在此处修改训练的epoch数
        ...
        if args.stage == 1:
            ...
            self.rrf_stage='background' #optimize background,optimize all,optimize mask
        self.background_optimize_epoch=conf.get_int("background_epoch", 1)
        self.mask_optimize_epoch=conf.get_int("mask_epoch", 0)
        self.all_optimize_epoch=conf.get_int("all_epoch", 1)
    ...
    def start(self):
        ...
        ior_data=f'tet_visual/{self.exp_name}/ior_list.txt'
        cmd=f'rm {ior_data}'
        os.system(cmd)
        for epoch in range(self.num_epochs):

            if self.args.stage==2:
                total_epoch_one_round=self.background_optimize_epoch+self.mask_optimize_epoch+self.all_optimize_epoch
                step_now=epoch%total_epoch_one_round
                if step_now<self.background_optimize_epoch:
                    self.rrf_stage='background'
                elif step_now<self.background_optimize_epoch+self.all_optimize_epoch:
                    self.get_ior(10)
                    self.rrf_stage='all'
                else:
                    self.rrf_stage='mask'
            ...
            batch = 0
            for b in range(self.num_epoch_repeats):
                for data in self.train_data_loader:
                    
                    losses = self.train_step(data, global_step=epoch)
                    ...
                    if batch % self.save_interval == 0 and (epoch > 0 or batch > 0) or batch == self.num_epochs*len(self.train_data_loader)*self.num_epoch_repeats-1:
                        print("saving")
                    ...
                    if (
                        batch == self.num_total_batches - 1 #遍历完整个数据集
                        or batch % self.accu_grad == self.accu_grad - 1 #达到backward的次数
                    ):
                        self.optim.step()
