...
def extra_args(parser):
    ...
    parser.add_argument("--tet_scale", type=float, default=4.5, help="Scale of the tet")
    parser.add_argument(
        "--sphere_radius", type=float, default=1.0, help="Radius of the bounding sphere"
    )
    parser.add_argument("--ior", type=float, default=1.19, help="index of refraction")
    parser.add_argument("--exp_name", type=str, default='ball_diffior', help="index of refraction")
...
name=f"eval_result/{args.exp_name}"
...
dset = Dataset(args.datadir, stage="test",dataset_type=args.name[0:7] if args.name[0:7] =='eikonal'else 'blender',stage_=args.stage)
...
renderer = NeRFRenderer.from_conf(
    ...
    enable_refr=True,#args.enable_refr,
    enable_refl=True,#args.enable_refl,
    stage=args.stage,
    ...
    need_trace=True#args.need_trace,
).to(device=device)
...
with torch.no_grad():
    index=0
    with open(f'{name}/me.txt','w') as f:
        ior_list=[1.0,1.1,1.3,1.5,1.7,1.9,2.1]
        for data in data_loader:
            ...
            center=data["center"].to(device=device)
            cam_rays = util.gen_rays(pose, W, H, focal, z_near, z_far,c=center)  # (H, W, 8)
            rgbs_gt = image * 0.5 + 0.5  # (3, H, W)

            import copy

            mask_added=copy.deepcopy(mask).permute(1,2,0).to(device)
            cam_rays=torch.cat([cam_rays,mask_added],dim=-1)
            import time
            ...
            for ior in ior_list:
                ...
                #cv2.imwrite(f"{name}/mask_{index}.png", mask.squeeze(0).unsqueeze(-1).cpu().numpy() * 255)
                
                cv2.imwrite(
                    f"{name}/mask_rgbs_{index}_{ior}.png",
                    cv2.cvtColor((rgbs.squeeze(0) * mask).permute(1, 2, 0).cpu().numpy() * 255,cv2.COLOR_BGR2RGB)
                )
                cv2.imwrite(
                    f"{name}/mask_rgbs_gt{index}.png",
                    cv2.cvtColor((rgbs_gt.squeeze(0) * mask).permute(1, 2, 0).cpu().numpy() * 255,cv2.COLOR_BGR2RGB)
                )
                cv2.imwrite(
                    f"{name}/rgbs_gt{index}.png",
                    cv2.cvtColor(rgbs_gt.squeeze(0).permute(1, 2, 0).cpu().numpy() * 255,cv2.COLOR_BGR2RGB)
                )
                cv2.imwrite(
                    f"{name}/rgbs_{index}_{ior}.png",
                    cv2.cvtColor(rgbs.squeeze(0).permute(1, 2, 0).cpu().numpy() * 255,cv2.COLOR_BGR2RGB)
                )

                psnr_mask = calculate_psnr(
                    rgbs_gt.squeeze(0).permute(1, 2, 0).cpu().numpy(),
                    rgbs.squeeze(0).permute(1, 2, 0).cpu().numpy(),
                    mask.permute(1, 2, 0).cpu().numpy(),
                )
                ssim_mask = calculate_ssim(
                    rgbs_gt.squeeze(0).permute(1, 2, 0).cpu().numpy(),
                    rgbs.squeeze(0).permute(1, 2, 0).cpu().numpy(),
                    mask.permute(1, 2, 0).cpu().numpy(),
                )

                rgbs_mask=(rgbs.squeeze(0).permute(1, 2, 0).cpu().numpy())*(mask.permute(1, 2, 0).cpu().numpy())
                rgbs_gt_mask=(rgbs_gt.squeeze(0).permute(1, 2, 0).cpu().numpy())*(mask.permute(1, 2, 0).cpu().numpy())
                rgbs_mask=torch.tensor(rgbs_mask).permute(2,0,1).unsqueeze(0)
                rgbs_gt_mask=torch.tensor(rgbs_gt_mask).permute(2,0,1).unsqueeze(0)
                lpips_mask = compare_lpips(rgbs_mask, rgbs_gt_mask).sum().item()
            
                lpips = compare_lpips(rgbs, rgbs_gt).sum().item()
                rgbs1=rgbs.squeeze(0).permute(1, 2, 0).cpu().numpy()
                rgbs_gt1=rgbs_gt.squeeze(0).permute(1, 2, 0).cpu().numpy()
                ssim=compare_ssim(rgbs1,rgbs_gt1,multichannel=True,data_range=1,channel_axis=2)
                ...
                total_psnr_mask += psnr_mask
                total_lpips_mask += lpips_mask
                ...
                f.write(f'psnr:{psnr},ssim:{ssim},lpips:{lpips}\n')
                f.write(f'psnr_mask:{psnr_mask},ssim_mask:{ssim_mask},lpips_mask:{lpips_mask}\n')
            f.write(f'final psnr:{total_psnr / cnt},final ssim:{total_ssim / cnt},final lpips:{total_lpips / cnt}\n')
            f.write(f'final mask psnr:{total_psnr_mask / cnt},final ssim:{total_ssim_mask / cnt},final lpips:{total_lpips_mask / cnt}\n')
print(
